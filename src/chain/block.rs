...
fn get_id_to_transaction(id: &str, header: &str, network: Network) -> (Integer, Header) {
    let s = string_to_bytes(header).unwrap();
    let h = Header::deserialize(&s, network).unwrap();
...
pub static TESTNET: Lazy<HashMap<Integer, Header>> = Lazy::new(|| {
    let mut h: HashMap<Integer, Header> = HashMap::new();
