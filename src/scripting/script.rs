...
use crate::{
    ...
    transaction::varint::{varint_decode, varint_encode},
};
...
impl Script {
    pub fn deserialize(data: &[u8]) -> Result<Self, ScriptError> {
        ...
        match varint_decode(data, 0) {
            Err(_) => return Err(ScriptError::InvalidScript),
            Ok(var_int) => {
...
mod script_test {
    ...
    fn deserialize() {
        ...
        assert_eq!(s[0], Operation::Element(string_to_bytes("3045022000eff69ef2b1bd93a66ed5219add4fb51e11a840f404876325a1e8ffe0529a2c022100c7207fee197d27c618aea621406f6bf5ef6fca38681d82b2f06fddbdce6feab601")));
        assert_eq!(s[1], Operation::Element(string_to_bytes("04887387e452b8eacc4acfde10d9aaf7f6d9a0f975aabb10d006e4da568744d06c61de6d95231cd89026e286df3b6ae4a894a3378e393e93a0f45b666329a0ae34")));
        assert_eq!(s[2], Operation::Command(OP_CHECKSIG));
    }
}
